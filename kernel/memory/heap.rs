...
impl KernelAllocator {
    pub const fn new() -> Self {
        KernelAllocator { heap: RefCell::new(None) }
    }
...
unsafe impl<'a> Alloc for &'a KernelAllocator {
    unsafe fn alloc(&mut self, layout: Layout) -> Result<*mut u8, AllocErr> {
        off();
        let ret = self.heap.borrow_mut().as_mut().unwrap().malloc(layout.size(), layout.align());
        on();
        Ok(ret)
    }
    ...
    unsafe fn dealloc(&mut self, ptr: *mut u8, layout: Layout) {
        off();
        let ret = self.heap.borrow_mut().as_mut().unwrap().free(ptr, layout.size());
        on();
        ret
    }
}
